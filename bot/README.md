...
بعد إرسال الصور يعرض Telegram قائمة متعددة الاختيار:

- `NocoDB`: نشر المنتج في قاعدة الموقع.
- `Tifawt`: إرسال الاسم والمرجع والسعر والصور الأصلية فقط.
- `Jumia`: نشر النسخة الاحترافية في Jumia وإضافة صف الشيت.

لا تُنفّذ إلا الوجهات المحددة. عند اختيار Jumia بدون NocoDB يُنشأ سجل تقني
`PAUSED` في NocoDB لحفظ الصور الدائمة، لكنه لا يظهر في الموقع.

### المنتجات متعددة الألوان

- المسار المفضّل للمنتج الموجود: اضغط `🎨 إضافة ألوان لمنتج موجود`، أرسل
  المرجع، ثم أرسل صورة أو ألبوم صور الألوان. هذا المسار لا يعيد إنشاء المنتج
  ولا يغيّر وصفه أو صوره الأساسية.
- يستخرج الذكاء الأشكال اللونية الظاهرة من الصور. التركيبة مثل `Blanc et Noir`
  تُعامل كلون واحد، وليست لونين منفصلين.
- يطلب Telegram تأكيد القائمة قبل أي توليد إضافي مدفوع. عند الرفض ينتظر قائمة
  يدوية مفصولة بفواصل.
- يبقى المنتج سجلاً واحداً في NocoDB وTifawt، بينما يُنشأ منتج Jumia مستقل
  لكل لون بصورة 1080×1080.
- SKU اللون يحمل لاحقة مميزة مثل `ERY-WATCH-JCBCNO`. كل مسارات طلبات Jumia
  تحذف `ERY-`/`ER-` ولاحقة اللون المعلّمة `-JC…` فقط، فيستلم Tifawt المرجع `WATCH`.
- صور الأمام/الظهر والتفاصيل تبقى في جدول المنتجات. صور كل لون تُحفظ في جدول
  `ProductVariants` (`my006z3z2ataq7u`) وتظهر كاختيارات ألوان في صفحة المنتج.
- لا تُعطّل الألوان القديمة قبل نجاح نشر لون بديل في Jumia. إلغاء العملية أو
  فشل مزود الذكاء/Jumia يترك الألوان المنشورة سابقاً كما هي.
- اضبط `NOCODB_TABLE_PRODUCT_VARIANTS` و`VITE_NOCODB_TABLE_PRODUCT_VARIANTS`
  إذا تغيّر معرّف الجدول.

### مركز تحكم البوت في Dashboard

- افتح لوحة الإدارة ثم `مركز تحكم البوت` لتعديل سير الموافقات، وجهات النشر
  الافتراضية، نماذج الذكاء، معالجة الصور وقيم Jumia.
- الإعدادات تُحفظ في سجل تقني مخفي `ERY-BOT-SETTINGS` داخل جدول المنتجات
  بحالة `NO POSTEBL` وبصيغة AES-GCM مشفرة وموقعة، لذلك تتشاركها خدمة الموقع
  وخدمة Telegram وتبقى بعد النشر دون كشفها أو السماح بتزويرها من الواجهة.
- مفاتيح Telegram وNocoDB وJumia وموفري الذكاء لا تُرسل إلى المتصفح؛ تعرض
  اللوحة حالة الاتصال فقط. اضبط `ADMIN_PASSWORD` و`BOT_SETTINGS_ENCRYPTION_KEY`
  بالقيم الخاصة نفسها في خدمتي الموقع والبوت داخل EasyPanel.
